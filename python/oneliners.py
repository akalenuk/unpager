...
def error(A, B, X):
	return sum([abs(di) for di in add(mul(A, X), scale(B, -1.0))])

def solve(A, B, Xi):
	return Xi if error(A, B, Xi) < 0.0001 else solve(A[1:] + [A[0]], B[1:] + [B[0]], project(Xi, A[0], B[0]))

def invert(A):
	return [solve(A, ort, [0.0]*len(A)) for ort in ident(len(A))]
